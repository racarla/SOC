...
void ExciteMgr::Init()
{
  ...
  VecChan tVecStart_s, tVecDur_s, tVecOnePulse_s, ampVec_nd;
  ...
  tStart_s = 1.0; tDur_s = 20.0;

  numChan = 1;
  numElem = 46;

  tVecStart_s.setConstant(numChan, tStart_s);
  tVecDur_s.setConstant(numChan, tDur_s);

  exciteTest01_.Init(kOMS, tVecStart_s, tVecDur_s, freqMat_rps.block(0, 0, 1, numElem), phaseMat_rad.block(0, 0, 1, numElem), ampMat_nd.block(0, 0, 1, numElem));

  // Excitation 2: Orthogonal Multi-Sine, Ailerons
  exciteTest02_.Init(kOMS, tVecStart_s, tVecDur_s, freqMat_rps.block(1, 0, 1, numElem), phaseMat_rad.block(1, 0, 1, numElem), ampMat_nd.block(1, 0, 1, numElem));

  // Excitation 3: Orthogonal Multi-Sine, Rudder
  exciteTest03_.Init(kOMS, tVecStart_s, tVecDur_s, freqMat_rps.block(2, 0, 1, numElem), phaseMat_rad.block(2, 0, 1, numElem), ampMat_nd.block(2, 0, 1, numElem));

  // Excitation 4: Orthogonal Multi-Sine, Elevator and Ailerons
  tStart_s = 1.0; tDur_s = 20.0; amp_nd = 4 * kD2R;
  ...
  exciteTest04_.Init(kOMS, tVecStart_s, tVecDur_s, freqMat_rps.block(0, 0, 2, numElem), phaseMat_rad.block(0, 0, 2, numElem), ampMat_nd.block(0, 0, 2, numElem));
  ...
  exciteTest05_.Init(kOMS, tVecStart_s, tVecDur_s, freqMat_rps, phaseMat_rad, ampMat_nd);
  ...
  exciteTest06_.Init(kLinear, tVecStart_s, tVecDur_s, freqVecStart_rps, freqVecEnd_rps, ampVecStart_nd, ampVecEnd_nd);

  // Excitation 7: Chirp, Aileron
  exciteTest07_.Init(kLinear, tVecStart_s, tVecDur_s, freqVecStart_rps, freqVecEnd_rps, ampVecStart_nd, ampVecEnd_nd);

  // Excitation 8: Chirp, Rudder
  exciteTest08_.Init(kLinear, tVecStart_s, tVecDur_s, freqVecStart_rps, freqVecEnd_rps, ampVecStart_nd, ampVecEnd_nd);
  ...
  exciteTest09_.Init(kLinear, tVecStart_s, tVecDur_s, freqVecStart_rps, freqVecEnd_rps, ampVecStart_nd, ampVecEnd_nd);

 // Excitation 10: Doublet, Elevator
  // tStart_s = 1.0; tOnePulse_s = 0.23; amp_nd = 4 * kD2R; // 23 m/s
  tStart_s = 1.0; tOnePulse_s = 0.27; amp_nd = 4 * kD2R; // 17 m/s

  numChan = 1;

  tVecStart_s.setConstant(numChan, tStart_s);
  tVecOnePulse_s.setConstant(numChan, tOnePulse_s);
  ampVec_nd.setConstant(numChan, amp_nd);

  exciteTest10_.Init(kDoublet, tVecStart_s, tVecOnePulse_s, ampVec_nd);

  // Excitation 11: Doublet, Aileron
  // tStart_s = 1.0; tOnePulse_s = 0.54; amp_nd = 4 * kD2R; // 23 m/s
  tStart_s = 1.0; tOnePulse_s = 0.72; amp_nd = 4 * kD2R; // 17 m/s
  ...
  exciteTest11_.Init(kDoublet, tVecStart_s, tVecOnePulse_s, ampVec_nd);

  // Excitation 12: Doublet, Rudder
  exciteTest12_.Init(kDoublet, tVecStart_s, tVecOnePulse_s, ampVec_nd);
  ...
  exciteTest13_.Init(kPulse, tVecStart_s, tVecOnePulse_s, ampVec_nd);
  ...
  exciteTest14_.Init(kPulse, tVecStart_s, tVecOnePulse_s, ampVec_nd);
  // Excitation 15: Pulse, Rudder

  exciteTest15_.Init(kPulse, tVecStart_s, tVecOnePulse_s, ampVec_nd);

  // Excitation 16: Pulse, Aileron (wider)
  // tStart_s = 1.0; tOnePulse_s = 0.6; amp_nd = 25 * kD2R; // 23 m/s
  tStart_s = 1.0; tOnePulse_s = 0.6; amp_nd = 4 * kD2R; // 17 m/s
  ...
  exciteTest16_.Init(kPulse, tVecStart_s, tVecOnePulse_s, ampVec_nd);
  ...
  exciteTest17_.Init(kDoublet3211, tVecStart_s, tVecOnePulse_s, ampVec_nd);
  ...
  exciteTest18_.Init(kDoublet3211, tVecStart_s, tVecOnePulse_s, ampVec_nd);

  // Excitation 19: Doublet3211, Rudder
  exciteTest19_.Init(kDoublet3211, tVecStart_s, tVecOnePulse_s, ampVec_nd);
}

ExciteMgrOut ExciteMgr::Compute(const bool& exciteMode, const uint8_t& indxTest, const float& t_s)
{
  exciteMgrOut_.exciteMode = exciteMode;
  exciteMgrOut_.indxTest = indxTest;
  exciteMgrOut_.tExcite_s = 0.0;

  exciteMgrOut_.cmdExcite.setZero(4);
  VecChan cmdExciteTemp(4);
  cmdExciteTemp.setZero();

  if (exciteMode == 1) {

    exciteMgrOut_.tExcite_s = t_s - tEngage_s_;

    switch (indxTest) {
      case 0:
        break;
      case 1:
        cmdExciteTemp = exciteTest01_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[1] = cmdExciteTemp[0];
        break;
      case 2:
        cmdExciteTemp = exciteTest02_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[0] = cmdExciteTemp[0];
        break;
      case 3:
        cmdExciteTemp = exciteTest03_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[2] = cmdExciteTemp[0];
        break;
      case 4:
        cmdExciteTemp = exciteTest04_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[0] = cmdExciteTemp[0];
        exciteMgrOut_.cmdExcite[1] = cmdExciteTemp[1];
        break;
      case 5:
        cmdExciteTemp = exciteTest05_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[0] = cmdExciteTemp[0];
        exciteMgrOut_.cmdExcite[1] = cmdExciteTemp[1];
        exciteMgrOut_.cmdExcite[2] = cmdExciteTemp[2];
        break;
      case 6:
        cmdExciteTemp = exciteTest06_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[1] = cmdExciteTemp[0];
        break;
      case 7:
        cmdExciteTemp = exciteTest07_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[0] = cmdExciteTemp[0];
        break;
      case 8:
        cmdExciteTemp = exciteTest08_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[2] = cmdExciteTemp[0];
        break;
      case 9:
        cmdExciteTemp = exciteTest09_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[0] = cmdExciteTemp[0];
        exciteMgrOut_.cmdExcite[1] = cmdExciteTemp[1];
        break;
      case 10:
        cmdExciteTemp = exciteTest10_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[1] = cmdExciteTemp[0];
        break;
      case 11:
        cmdExciteTemp = exciteTest11_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[0] = cmdExciteTemp[0];
        break;
      case 12:
        cmdExciteTemp = exciteTest12_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[2] = cmdExciteTemp[0];
        break;
      case 13:
        cmdExciteTemp = exciteTest13_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[1] = cmdExciteTemp[0];
        break;
      case 14:
        cmdExciteTemp = exciteTest14_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[0] = cmdExciteTemp[0];
        break;
      case 15:
        cmdExciteTemp = exciteTest15_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[2] = cmdExciteTemp[0];
        break;
      case 16:
        cmdExciteTemp = exciteTest16_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[0] = cmdExciteTemp[0];
        break;
      case 17:
        cmdExciteTemp = exciteTest17_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[1] = cmdExciteTemp[0];
        break;
      case 18:
        cmdExciteTemp = exciteTest18_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[0] = cmdExciteTemp[0];
        break;
      case 19:
        cmdExciteTemp = exciteTest19_.Compute(exciteMgrOut_.tExcite_s);
        exciteMgrOut_.cmdExcite[2] = cmdExciteTemp[0];
        break;
...
ExciteMgrLog ExciteMgr::Log(const ExciteMgrOut& ExciteMgrOut)
{
  ...
  exciteMgrLog.tExcite_s = ExciteMgrOut.tExcite_s;
