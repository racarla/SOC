...
void CtrlMgr::Config(const ObjJson &objJson) {
  ...
  // Create and Configure the Vehicle Definitions, convert them into a Map of Map floats.
  ...
  const ObjJson &objSignals = objJson["Signals"]; // Create Signals Object
  MapMapFloat mapSignals = Json2MapMapFloat(objSignals); // Convert the Json Object to the map container
  ...
  std::map <std::string, ScasSys> scasSysMap; // Create the Map of Scas Classes
  ...
  scasSysMap ScasSys::Config(objScas); // Configure System
